...
**normal の列は 4.1 で実機（Android）調整の確定値に置き換えた。compact の列は未確定**
（`TOKEN_METRICS.compact` の暫定値のまま）。詳細と全項目は 3-1。
...
**normal は 2026-09-08 に実機（Android）で確定した値。compact は未確定**
（`TOKEN_METRICS.compact` の暫定値。DevTools のデバイス表示で詰める）。
下の表は `TOKEN_METRICS` の**全項目**。

| 項目 | normal | compact | CSS 変数 | 意味 |
|---|--:|--:|---|---|
| `num` | 32 | 23 | `--numW` | 数字の箱幅 |
| `numMx` | 2 | 1 | `--numMx` | 数字の左右マージン。**スライダーが無い**（2.5 の実機調整の値のまま） |
| `numF` | 52 | 36 | `--numF` | 数字の文字サイズ（`.num`） |
| `op` | 30 | 29 | `--opW` | 演算子の箱幅 |
| `opF` | 32 | 26 | `--opF` | 演算子の文字サイズ（`.tok.op`） |
| `fac` | 19 | 16 | `--facW` | 階乗の箱幅 |
| `facF` | 39 | 26 | `--facF` | 階乗の文字サイズ（`.tok.fac`） |
| `facMl` | -3 | -2 | `--facMl` | 階乗の食い込み（`.tok.fac` の `margin-left`）。4.0 で定数 `FAC_ML` を廃止してここへ移した |
| `par` | 16 | 18 | `--parW` | 括弧の箱幅 |
| `parF` | 42 | 31 | `--parF` | 括弧の文字サイズ（`.tok.lp` `.tok.rp`） |
| `h` | 59 | 50 | `--tokH` | トークンの高さ |
| `chipF` | 38 | 26 | `--chipF` | トレイの記号の文字サイズ（`.chip`） |
| `trayGap` | 0 | 2 | `--trayGap` | トレイの `gap` |
| `trayPad` | 9 | 4 | `--trayPad` | トレイの `padding` |
| `btn` | 52 | 38 | `--btnW` | 丸ボタンの直径（`.iconbtn` `.plainbtn`） |
| `gear` | 42 | 38 | `--gearW` | `#gear` の直径 |
| `ic` | 28 | 24 | `--icW` | アイコン SVG の描画サイズ（`.ic`）。`btn` は箱しか変えないので別に持つ |

**compact が normal より大きい行（`par` 18 > 16）があるのは、compact が未確定だから。**
4.0 までは compact を「normal との比」で自動追従させていたが、normal を実機で確定させた
時点で比の分母が動かなくなり意味を失ったので、**4.1 で `SIZE_RATIO` を廃止して
normal / compact を別々に持つ形にした**（開発者パネルは今の画面のモードのほうを直接編集する。
6-5）。compact を確定させたらこの表の compact 列を書き直す。
...
```
4×(num + 2×numMx) + 3×op + 8×par + 8×(fac + facMl) = 490px（normal）／443px（compact）
```

例（探索が返した並び。実際に組める）: `((((5!)!)!)!)!+8!+3!+6!`

このときの縮小率は**画面幅で変わる。** 分母の「使える幅」（`#exprwrap` の実測幅）が
画面幅で決まるためで、**狭い端末ほど縮む。** 4.1 の確定値での実測は次のとおり
（`#app` の `max-width` が 430px なので、画面がそれより広くても 430px の行で頭打ちになる）。

| 画面幅 | 使える幅（normal） | normal | 使える幅（compact） | compact |
|--:|--:|--:|--:|--:|
| 430px | 388px | **79.2%** | 392px | **88.5%** |
| 390px | 348px | 71.0% | 352px | 79.5% |
| 360px | 318px | 64.9% | 322px | 72.7% |

**「79.2% / 88.5%」と書くときは画面幅 430px の値。** 下限 50% には 360px でも届かない。
...
（`h3 .mk`）。状態は `G.devSecs`（`devVars` とは別。1=開 / 0=閉）に保存する。既定は
**SIZE-TRIAL の 3 節だけ開き、他は閉じる** ―― スライダーが 23 本あり、compact を確認する
画面高 690px では全部開いたままだと目的の 1 本にたどり着けないため。
...
**折りたたむ単位は「見出しから次の見出しまで」。** `SIZE-TRIAL（寸法・決定後に撤去）` の
見出しだけは `data-sec` を付けず**折りたためない** ―― これは節ではなく、直後に続く
3 節（式のトークン／トレイ／ボタン・アイコン）をまとめる帯だから。逆に、パネル先頭の
枠・吸着のスライダー群は見出しを持たない塊だったので、`枠・吸着（基本）` の見出しを
与えて折りたためるようにした（23 本のうち 7 本がここにある）。

**編集中のモードを見出しに出す（4.1）。** SIZE-TRIAL の帯の見出しに `#dv-mode` を置き、
`normal を編集中` / `compact を編集中` と書く。`#devhead` はボタン 3 つで幅が無く、
これ以上文字を足すと省略表示に食われて読めないため、ヘッダではなく節見出し側に出す。
画面の高さが境目をまたいだら `matchMedia(COMPACT_MQ)` の `change` で
`renderDevPanel()` を呼び、表示とスライダーの値を切り替える。
...
2. **タップ領域が 48px 未満（問題画面・ナビのみ）**: 4.1 の確定値で normal は
   `.iconbtn` `.plainbtn` が 52px で**満たすようになった**が、`#gear` は 42px で
   **まだ満たさない**。`ZONE_MIN` 20px も未達（吸着の catch 半径 40px で補う）。
   **compact は未確定**（暫定値では `.iconbtn` 38px / `#gear` 38px でどちらも未達）。
