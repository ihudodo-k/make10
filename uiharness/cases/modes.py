...
def run(ui):
    ...
    ui.check("CHAL の件数", ui.ev("CHAL.length"), 1929)
    ui.check("FREE の件数", ui.ev("FREE.length"), 18797)
    ui.check("合計", ui.ev("COURSE.length+FREE.length+CHAL.length"), 21726)
    ui.check("コードの索引も同じ数", ui.ev("INDEX.size"), 21726)
    # ── 本編 ──────────────────────────────────────────────
    ui.ev("start('course')")
    ...
    ui.check("挑戦の問題番号", ui.text("pmode"), "挑戦 1/1929")
    ...
    ui.check("残りは 全問 − 出題済み",
             ui.ev("CHAL.length-G.chalShown.length"), 1929 - 41)
    ui.check("挑戦の問題番号は出題数で進む", ui.text("pmode"), "挑戦 41/1929")
