...
**5.1 で枠も揃えた。** 4.5 で絵柄は揃ったが、**枠の有無がまだ違っていた** ――
ホームの `#gear` は `button` の既定（`1px solid var(--edge)` ＋ `background:var(--panel)`
＋ 角丸 11px）が効いた**面のあるボタン**、問題画面の `#menu` は `.plainbtn` で
**線だけ**、という食い違い。`#gear` に `.plainbtn` を付けて枠と地を外し、
`#gear` の CSS は**位置だけ**（`position:absolute` / `top:12px` / `right:12px` /
`z-index:50`）に減らした。大きさ・枠・地・透明度は `.plainbtn` の 1 か所が持つ。

**大きさは `--btnW` に一本化した。** `#gear` は独自の `--gearW`（42 / 38px）を
持っていて `#menu`（`--btnW` = 52 / 44px）より小さかった。`.plainbtn` に寄せたので
**タップ領域は 42→52 / 38→44 に広がる**（縮まない）。参照が 0 件になった
`--gearW` と `TOKEN_METRICS` の `gear` は削除した。

**三本線の定義は `ICON_MENU` 1 つだけ。** `#menu`（問題画面）にも `#gear`（ホーム）にも
初期化時に同じ文字列を流し込む。同じ絵柄が 2 つの定義を持つと片方だけ直す事故が起きる。
5.1 で `#gear` の役割が「設定」に固定されたので、`go()` による差し替えは無くなった
（戻る矢印は左上の `#navback` が別に持つ。GAME-SPEC 5-0）。
...
#### `.iconbtn` と `.plainbtn` の使い分け

| class | 見た目 | 使う所 |
|---|---|---|
| `.iconbtn` | `--btnW` 角、**丸い枠**（`border-radius:50%` ＋ `1.5px` の線）、地なし | 問題画面の操作 ―― `#back`（ホーム）/ `#clear`（全部消す）/ `#hint`（ヒント） |
| `.plainbtn` | `--btnW` 角、**枠も地も無し**、`opacity:.9` | ナビゲーション ―― `#menu`（問題画面の三本線）/ `#gear`（設定）/ `#navback`（戻る） |

**枠の有無が「操作か、移動か」を分ける。** 盤面を触る操作は丸で囲って押せることを
示し、画面を移るだけのものは線だけにして前に出さない。大きさ（`--btnW`）は
どちらも同じなので、タップ領域は揃う。
...
2. **タップ領域が 48px 未満（問題画面・ナビのみ）**: 4.3 の確定値での現状は次のとおり。
   ...
   `#gear` / `#navback` も 5.1 で `.plainbtn` に揃えたので同じ（**normal 52px / compact 44px**）。
   5.0 までの `#gear` は 42 / 38px でどちらも未達だったが、**5.1 で normal は満たすようになった**。
