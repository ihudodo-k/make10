...
def run(ui):
    ...
    ui.check("CHAL の件数", ui.ev("CHAL.length"), 1268)
    ui.check("FREE の件数", ui.ev("FREE.length"), 15586)
    ui.check("合計", ui.ev("COURSE.length+FREE.length+CHAL.length"), 17854)
    ui.check("コードの索引も同じ数", ui.ev("INDEX.size"), 17854)
